describe("Footer", () => {
  ...
  const projectGithubLink =
    "https://github.com/CanopusFalling/Queer-Calendar-Sheffield";
  ...
  pathsToTest.forEach((path) => {
    context(`Testing on ${path}`, () => {
      ...
      it(`should show text about contributors`, () => {
        cy.get(footerSelector).contains(
          `This calendar is built and maintained by our contributors. You can also get involved below.`,
        );
      });
      ...
      it(`should contain a link to the google form for adding an event`, () => {
        cy.get(footerSelector)
          ...
          .should("have.attr", "href", googleFormLink);
      });

      it(`should contain a link to the github`, () => {
        cy.get(footerSelector)
          .contains(`View Project on GitHub`)
          .should("have.attr", "href", projectGithubLink);
      });
