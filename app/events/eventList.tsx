...
export default async function EventList({ requestArguments }: EventListProps) {
    ...
    const eventCards = events.items.map((event: Event) => {
        ...
        return (
            <div key={id} className="block rounded-lg bg-white mb-2 p-6 shadow dark:shadow-white/10 dark:bg-neutral-700">
                ...
                {location && (
                    <p className="mb-2 leading-tight text-neutral-800 dark:text-neutral-50">
                        <b>Location: </b>{" "}
                        <a href={`https://www.google.com/maps/dir/?api=1&destination=${urlEncodedLocation}`} target="_blank">
                            {location}
                        </a>
                    </p>
                )}
                <div className="mb-4 text-base text-neutral-600 break-words dark:text-neutral-200" dangerouslySetInnerHTML={{ __html: sanitizedDescription }}
                />
    ...
    return <main className="p-6 dark:bg-neutral-800" role="main"><h1 class="text-3xl">Events</h1>{eventCards}</main>;
}
