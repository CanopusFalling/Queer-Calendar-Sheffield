...
class CalendarSource {
  ...
  #fetchGoogleCalendarData(timeMin, timeMax, showDeleted) {
    ...
    let queryString = new URLSearchParams(
      Object.entries(parameters).filter(([key, value]) => value !== undefined),
    ).toString();
...
class CalendarEvent {
  ...
  #google_ingest_time(data) {
    if (data.start != undefined) {
      ...
      } else if (data.start.date != undefined) {
    ...
    if (data.end != undefined) {
      ...
      } else if (data.end.date != undefined) {
  ...
  toModal() {
    return `
        <div class="fixed inset-0 flex items-center justify-center z-50" id="eventModal-${
          this.id
        }" aria-labelledby="eventModalLabel-${this.id}" aria-hidden="true">
...
const SOURCES = [
  new CalendarSource(
    "Queer Calendar Sheffield Google Calendar",
    CalendarSourceType.GOOGLE_CALENDAR,
    "queercalendarsheffield@gmail.com",
  ),
];
...
document.addEventListener("DOMContentLoaded", function () {
  page_calendar.fetchAllEvents().then(() => {
